...
class AutoGenerator {
    ...
    generateText() {
        ...
        tableNames.forEach(table => {
            ...
            const tableName = (0, types_1.recase)(this.options.caseModel, tableNameOrig, this.options.singularize);
            if (this.options.lang === 'ts') {
                ...
                let tableTypeOverride;
                let tableVirtualFields;
                ...
                str += this.addTypeScriptFields(table, true, tableVirtualFields, tableTypeOverride, typeOverrides === null || typeOverrides === void 0 ? void 0 : typeOverrides.nullableFieldType) + "}\n\n";
                ...
                str += this.addTypeScriptFields(table, false, tableVirtualFields, tableTypeOverride, typeOverrides === null || typeOverrides === void 0 ? void 0 : typeOverrides.nullableFieldType);
                str += "\n" + associations.str;
    ...
    addTable(table) {
        ...
        if (this.options.virtualFields) {
            ...
            lodash_1.default.each(virtualColumns, (columnOptions, columnName) => {
    ...
    addVirtualField(columnName) {
        const space = this.space;
        const fieldName = (0, types_1.recase)(this.options.caseProp, columnName);
        let str = this.quoteName(fieldName) + ": {\n";
    ...
    getTypeScriptTableOverrideImports(tableTypeOverride) {
        const columnTypeOverridesByType = {};
        // type should only be imported once for each file
        lodash_1.default.keys(tableTypeOverride).forEach((columnName) => {
        ...
        // import per source
        const imports = {};
        lodash_1.default.keys(columnTypeOverridesByType).forEach((type) => {
        ...
        const importStrArr = [];
        lodash_1.default.keys(imports).forEach((source) => {
    ...
    addTypeScriptFields(table, isInterface, tableVirtualFields, tableTypeOverride, nullableFieldType) {
        const sp = this.space[1];
        const fields = lodash_1.default.keys(this.tables[table]);
        let str = '';
        const notOptional = isInterface ? '' : '!';
        fields.forEach(field => {
            let columnTypeOverride;
            if (tableTypeOverride) {
            ...
            if (columnTypeOverride && columnTypeOverride.isOptional !== undefined) {
                ...
                isOptional = fieldObj.allowNull && nullableFieldType !== types_1.NullableFieldTypes.Null;
            }
            str += `${sp}${name}${isOptional ? '?' : notOptional}: ` +
                `${columnTypeOverride && columnTypeOverride.type !== undefined ? columnTypeOverride.type : (this.getTypeScriptType(table, field) + (fieldObj.allowNull && nullableFieldType !== types_1.NullableFieldTypes.Optional ? " | null" : ""))};\n`;
        });
        lodash_1.default.each(tableVirtualFields, (columnOptions, columnName) => {
            const name = this.quoteName((0, types_1.recase)(this.options.caseProp, columnName));
