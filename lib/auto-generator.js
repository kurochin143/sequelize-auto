...
class AutoGenerator {
    ...
    generateText() {
        ...
        tableNames.forEach(table => {
            ...
            if (this.options.lang === 'ts') {
                ...
                str += this.addTypeScriptFields(table, false, tableTypeOverride);
                str += "\n" + associations.str;
    ...
    addTypeScriptFields(table, isInterface, tableTypeOverride) {
        const sp = this.space[1];
        const fields = lodash_1.default.keys(this.tables[table]);
        let str = '';
        ...
        fields.forEach(field => {
            ...
            const fieldObj = this.tables[table][field];
            str += `${sp}${name}${(columnTypeOverride === null || columnTypeOverride === void 0 ? void 0 : columnTypeOverride.isOptional) ? '?' : notOptional}: ${columnTypeOverride ? columnTypeOverride.type : (this.getTypeScriptType(table, field) + (fieldObj.allowNull ? " | null" : ""))};\n`;
        });
        return str;
    }
